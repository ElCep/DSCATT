...
class GiniExtension extends api.DefaultClassManager {
  def load(manager: api.PrimitiveManager) {
    manager.addPrimitive("gini", Gini)
  }
}

object Gini extends api.Reporter {
  ...
  def gini(l: Seq[Double]) = {
    if(l.size == 0) new api.ExtensionException("List should not be empty.")
  ...
  def report(args: Array[api.Argument], context: api.Context): AnyRef = {
    val l = args(0).getList.map(_.asInstanceOf[Double])
    gini(l).toLogoObject
  }
}
