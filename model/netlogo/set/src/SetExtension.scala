...
class SetExtension extends api.DefaultClassManager {
  def load(manager: api.PrimitiveManager) {
    manager.addPrimitive("diff", Diff)
  }
}

object Diff extends api.Reporter {
  // Takes a number and a list
  // Returns the first n values in a given list
  override def getSyntax = reporterSyntax(right = List(ListType, ListType), ret = ListType)

  def report(args: Array[api.Argument], context: api.Context): AnyRef = {
    val l1 = args(0).getList
    val l2 = args(1).getList.toSet
    l1.filter(e => !l2.contains(e)).toLogoList
  }
}
